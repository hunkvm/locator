...
type Transport struct {
	...
	tlsLoader *security.GRPCCredsLoader
}

func NewTransport(logger *zap.Logger, loader *security.GRPCCredsLoader) *Transport {
	return &Transport{
		logger:    logger,
...
func (s *Transport) AddPeer(id uint64, addr string) {
	...
	if s.tlsLoader != nil {
		...
		if err != nil {
			s.logger.Error("failed to split host port", zap.Error(err))
			return
		}
		if host == "" {
			host = addr
		}
		creds, err = s.tlsLoader.ClientCredentials(host)
		if err != nil {
			s.logger.Error("failed to load client credentials", zap.Error(err))
			return
		}
	} else {
