...
func (m *CacheManager) ApplyServices(ns string, srvs []*types.Service) {
	...
	for serviceName, instances := range groups {
		...
		routes[routeConfigName] = buildRouteConfig(routeConfigName, serviceName, serviceRoutes)
		listeners[serviceName] = buildListener(serviceName, routeConfigName)
	}
...
func (m *CacheManager) UpsertService(ns string, svc *types.Service) {
	...
	m.cache.UpsertResource(ns, resource.RouteType, rcName, buildRouteConfig(rcName, svc.Name, serviceRoutes))
	m.cache.UpsertResource(ns, resource.ListenerType, svc.Name, buildListener(svc.Name, rcName))
	m.logger.Debug("created route and listener", zap.String("namespace", ns),
		zap.String("route", rcName), zap.String("listener", svc.Name))
}

func (m *CacheManager) EvictService(ns string, svc *types.Service) {
	cn := clusterName(ns, svc.Name, svc.Version)
	addr, loc := svc.Address, svc.Metadata.GetLocality()
...
func newLbEndpoint(svc *types.Service) *endpointv3.LbEndpoint {
	...
	host, p, _ := net.SplitHostPort(svc.Address)
