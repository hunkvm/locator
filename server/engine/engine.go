...
type EngineConfig struct {
	...
	TransportTLSLoader *security.GRPCCredsLoader
}
...
func NewLocatorEngine(cfg EngineConfig) (*LocatorEngine, error) {
	...
	addrs := strings.Split(cfg.RaftConfig.PeerAddrs, ",")
	...
	if tick := cfg.RaftConfig.ElectionTick; tick == 0 {
		raftConfig.electionTick = defaultElectionTick
	} else {
		raftConfig.electionTick = tick
	}

	if duration := cfg.RaftConfig.Heartbeat; duration == 0 {
	...
	locatorEngine := &LocatorEngine{
		...
		idGenerator:   idgen.New(nodeID),
	}

	if count := cfg.RaftConfig.SnapshotCount; count == 0 {
		locatorEngine.snapshotCount = defaultSnapshotCount
	} else {
		locatorEngine.snapshotCount = count
	}

	if count := cfg.RaftConfig.CatchupEntries; count == 0 {
