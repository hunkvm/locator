...
type LocatorServer struct {
	...
	registryLoader  *security.GRPCCredsLoader
	transportLoader *security.GRPCCredsLoader
}
...
func NewLocatorServer(cfg config.ServerConfig, logger *zap.Logger) *LocatorServer {
	...
	var registryLoader, transportLoader *security.GRPCCredsLoader
	var registryOpts, transportOpts []grpc.ServerOption

	if cs := cfg.ClientChannelSecurity; cs.TLSPolicy != security.TLSPolicyNone {
		registryLoader = &security.GRPCCredsLoader{
			TLSPolicy:         cs.TLSPolicy,
			CaCertificateFile: cs.CaCertificateFile,
			CertificateFile:   cs.CertificateFile,
			PrivateKeyFile:    cs.PrivateKeyFile,
			RefreshInterval:   cs.RefreshInterval,
			OnRefreshError: func(err error) {
				msg := "failed to refresh registry TLS"
				logger.Error(msg, zap.Error(err))
			},
		}
		if crd, err := registryLoader.ServerCredentials(); err == nil {
			registryOpts = append(registryOpts, grpc.Creds(crd))
		} else {
			logger.Fatal("failed to create registry TLS", zap.Error(err))
		}
	}
	if cs := cfg.PeerChannelSecurity; cs.TLSPolicy != security.TLSPolicyNone {
		transportLoader = &security.GRPCCredsLoader{
			TLSPolicy:         cs.TLSPolicy,
			CaCertificateFile: cs.CaCertificateFile,
			CertificateFile:   cs.CertificateFile,
			...
			RefreshInterval:   cs.RefreshInterval,
			OnRefreshError: func(err error) {
				msg := "failed to refresh transport TLS"
				logger.Error(msg, zap.Error(err))
			},
		}
		if crd, err := transportLoader.ServerCredentials(); err == nil {
			transportOpts = append(transportOpts, grpc.Creds(crd))
		} else {
			logger.Fatal("failed to create transport TLS", zap.Error(err))
		}
	}
	...
	locatorEngine, err := engine.NewLocatorEngine(
		engine.EngineConfig{
			DataDir: cfg.DataDir, RaftConfig: cfg.Raft, Logger: logger,
...
func (r *LocatorServer) Start() error {
	...
	clientListenerAddr := r.config.Listeners.Client
	clientListener, err := net.Listen("tcp", clientListenerAddr)
	if err != nil {
		return fmt.Errorf("failed to listen on registration port: %w", err)
	}
	r.logger.Info("service registry listening", zap.String("address", clientListenerAddr))

	peerListenerAddr := r.config.Listeners.Peer
	peerListener, err := net.Listen("tcp", peerListenerAddr)
	if err != nil {
		return fmt.Errorf("failed to listen on transport port: %w", err)
	}
	r.logger.Info("raft transport listening", zap.String("address", peerListenerAddr))
	...
	go func() {
		...
		if err := r.registryServer.Serve(clientListener); err != nil {
			errors <- fmt.Errorf("failed to serve registry gRPC server: %w", err)
		}
	}()
	go func() {
		defer waitGroup.Done()
		if err := r.transportServer.Serve(peerListener); err != nil {
